import React from "react";
import { FaCamera, FaPenFancy, FaDisplay, FaVideo } from "react-icons/fa6";
import { TbBusinessplan } from "react-icons/tb";
import { MdAssuredWorkload } from "react-icons/md";
import CategoryCard from "./CategoryCard";

const dummyCategories = [
  {
    id: 0,
    name: "Design",
    icon: FaPenFancy,
    desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Sapiente mollitia, nulla doloribus quia aliquam, dolore ad deserunt nesciunt voluptates labore quod dolor reiciendis corrupti nihil.",
    color: "#24e7f5",
  },
  {
    id: 1,
    name: "Development",
    icon: FaDisplay,
    desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Sapiente mollitia, nulla doloribus quia aliquam, dolore ad deserunt nesciunt voluptates labore quod dolor reiciendis corrupti nihil.",
    color: "#27b1f1",
  },
  {
    id: 2,
    name: "Business",
    icon: TbBusinessplan,
    desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Sapiente mollitia, nulla doloribus quia aliquam, dolore ad deserunt nesciunt voluptates labore quod dolor reiciendis corrupti nihil.",
    color: "#6928ff",
  },
  {
    id: 3,
    name: "Marketing",
    icon: MdAssuredWorkload,
    desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Sapiente mollitia, nulla doloribus quia aliquam, dolore ad deserunt nesciunt voluptates labore quod dolor reiciendis corrupti nihil.",
    color: "#ff6529",
  },
  {
    id: 4,
    name: "Photography",
    icon: FaCamera,
    desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Sapiente mollitia, nulla doloribus quia aliquam, dolore ad deserunt nesciunt voluptates labore quod dolor reiciendis corrupti nihil.",
    color: "#ff2828",
  },
  {
    id: 5,
    name: "Acting",
    icon: FaVideo,
    desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Sapiente mollitia, nulla doloribus quia aliquam, dolore ad deserunt nesciunt voluptates labore quod dolor reiciendis corrupti nihil.",
    color: "#505050",
  },
];

function TopCategory() {
  return (
    <section className="bg-white flex flex-col gap-3 py-6 px-2 md:px-6 lg:px-8">
      <h2 className="text-md lg:text-xl font-semibold self-start">
        Choose favourite course from top category
      </h2>
      <div className="flex flex-wrap items-center justify-around py-6 gap-10">
        {dummyCategories.map((category, i) => (
          <CategoryCard category={category} key={i} />
        ))}
      </div>
    </section>
  );
}

export default TopCategory;
